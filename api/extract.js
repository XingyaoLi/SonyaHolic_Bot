...
2. 专属语境：推文很可能和泰国演员 Sonya Saranphat / LMSY 相关。如果内容里有“熊”相关名词（如 bear、หมี 等文字，请注意：这仅指文字代称，不包含表情符号）并且看起来像个人名，请替换成”Lookmhee“。
3. **表情符号保护**：原文中出现的所有 Emoji 表情（如 🐰, 🙂 等）必须被视为排版的一部分，绝对禁止删除、翻译或替换成 Lookmhee。必须 100% 保留它们，并保持它们在原文中的先后顺序。
4. 去除标签：忽略并删除推文原文中的所有带 # 号的话题标签。
5. 去除趋势关键词：删除全大写、单行出现的趋势打榜关键词（例如 SONYA BORN TO BE A STAR）。
6. 最终输出只需保留翻译后的中文正文以及原有的所有 Emoji，不要输出任何解释说明。
7. 必须强行按照要求的排版格式输出。`;
